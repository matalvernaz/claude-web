...
- **Transport**: `codex app-server` over JSON-RPC 2.0 stdio (the same server the Codex VS Code extension drives). The keyed pool keeps one account-control process per OpenAI slot for login/models/usage and one process per active chat for execution. Run-specific processes are intentional: app-server keeps an unsubscribed thread loaded for 30 minutes, so a process boundary is the only safe way to force A→B→A to reload B's appended turns. If a process dies, the live run gets an error event and the next message starts a fresh process and `thread/resume`s. Spawn and teardown share one per-key `asyncio.Lock` via `CodexAppServer._key_lock`, refcounted so the entry outlives every caller holding or awaiting it — popping the lock on teardown let a concurrent spawn publish into a key the caller had been told was closed, and let two spawns run for one key with the loser referenced by nothing. `_on_exit` terminates the child whenever the reader stops while it is still alive: it drops the pool entry, so leaving the process running would make it unreachable for both `close_key` and `shutdown_all`, surviving a unit restart. Personal app-servers also strip this app's own secrets from the child environment (`SCRUB_ENV_NAMES`, set from `CHILD_ENV_SCRUB`).
...
- **Driver**: `_codex_driver` in app.py mirrors the Claude driver's external contract (consumes `run.user_input_queue` with cancel/ack semantics, flips `between_turns`, publishes `run.client` so `/api/chat/stop` works — the shim's `interrupt()` issues `turn/interrupt`). Mid-turn user input goes through `turn/steer`, which **requires** `expectedTurnId` (`run.codex_turn_id`) as a precondition alongside `threadId` — omit it and the app-server rejects every mid-turn message with `-32600 missing field`, and the driver reports each as lost input. Its `input` array is built by the shared `_codex_input_items` so steer carries image attachments the same way `turn/start` does. Events translate in `codex_provider.item_events` to the exact v1 stream-json SSE shapes `_sdk_message_to_events` emits, so app.js renders both providers identically (`agentMessage` deltas → `partial_text`, `commandExecution` → Bash `tool_use`/`tool_result`, `turn/plan/updated` → `todos_update` (the `todoList` item branch is kept for older CLIs; no such item type exists as of 0.153), `plan`/`enteredReviewMode`/`exitedReviewMode`/`contextCompaction` items → text notes, `imageGeneration`/`imageView`/`subAgentActivity`/`collabAgentToolCall`/`dynamicToolCall` → generic tool chips (`dynamicToolCall` names itself `namespace__tool` and reads its output from `contentItems`; `functionCallOutput`, new in 0.153, is deliberately not rendered — it is an output with no matching call item, so a chip for it would dangle), `turn/completed` + `thread/tokenUsage/updated` → the `result` usage line and the context endpoint). Reasoning deltas are dropped on purpose — claude-web doesn't render thinking on either provider. `initialize` declares `capabilities.experimentalApi=true`, which unlocks `item/tool/requestUserInput`: bridged to the AskUserQuestion card via `_codex_question_decision` (answers map back by question id; skip/timeout/interrupt all send the empty answers mapping codex treats as "did not answer"; `isSecret` questions get a masked free-text field). A user message of exactly `/compact` maps to `thread/compact/start` instead of a turn; the `thread/compacted` notification and `contextCompaction` items dedupe via the driver's `compact_pending` flag. Sidebar search scans codex rollout files (`_scan_codex_sessions_for_query`, instruction/environment wrappers skipped) alongside Claude JSONLs.
