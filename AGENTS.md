...
- **Transport**: one shared `codex app-server` process (JSON-RPC 2.0 over stdio — the same server the Codex VS Code extension drives). The protocol is thread-scoped, so a singleton multiplexes every Codex conversation; `codex_provider.CodexAppServer` owns spawn/respawn, request/response correlation, per-thread notification queues, and per-thread approval handlers. If it dies, live runs get an error event and the next message respawns it and `thread/resume`s.
- **Why app-server and not `codex exec` / the `openai-codex-sdk` pip package**: interactive approvals. The exec path bakes an approval policy in at spawn; the app-server sends `item/commandExecution/requestApproval` / `item/fileChange/requestApproval` as JSON-RPC server→client requests mid-turn, which map 1:1 onto the existing permission-Future plumbing (`_gate_tool_permission`) and render in the same cards. Command approvals reuse the tool name "Bash" so `NO_SESSION_ALLOWLIST_TOOLS` coarse-signature rules apply; the wire answer is always plain accept/decline — claude-web's own per-session allowlist supplies "allow for session" semantics, so codex's `acceptForSession` is deliberately unused.
...
- **Models**: fetched live from `model/list` on the first `/api/providers` call and cached for the server process's life — nothing hardcoded, so new GPT releases appear without a deploy. Reasoning efforts ride the same effort picker (per-model lists, including codex-only levels like `ultra`).
- **Sessions**: codex thread ids double as claude-web session ids. `codex_session` (state.db) is the registry the sidebar/list/reopen/delete paths use; ownership/visibility reuses `session_owners` via the shared `emit()` init hook. Reopen history comes from `thread/read` — **codex 0.144 returns only message items there**, so a reopened codex session replays conversation text without tool chips (live streams show everything). `/api/chat` infers `provider=codex` from the registry when the field is missing, and 400s explicit cross-provider mismatches so the wrong backend can never resume a transcript.
...
- **Not supported with Codex** (capability-gated in the UI, 400 with a clear message server-side): plan mode, fork, file rewind/checkpoints, permission modes, per-user account slots (OpenAI auth is host-level), MCP page (configure in `~/.codex/config.toml`), background tasks, advisor combos. Personalities DO carry over (`developerInstructions` at thread start/resume; a mid-chat personality switch respawns onto the same thread like the Claude path). Claude credential toggles are inert for codex runs by design (slot pinned to "shared").
